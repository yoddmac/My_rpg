...
int *parsing_props_csv (int width, int height)
{
    ...
    char *buffer = NULL;
    size_t size = 0;
    FILE *fp = fopen("./survival_decor.csv" , "r");
    char *token = NULL;
    int x = 0;
...
vertex_t *initialisation_vertex(void)
{
    ...
    vertex->tiles_text = sfTexture_createFromFile("./map_survival.png", NULL);
    vertex->pos.x = 16;
    vertex->pos.y = 16;
    vertex->width = 120;
    vertex->height = 65;
...
void tiles_info(vertex_t *vertex, sfVertexArray *vertex_tab, int *parse_csv)
{
    for (unsigned int i = 0; i < vertex->width; i++)
        for (unsigned int j = 0; j < vertex->height; j++) {
            ...
            int tu = tileNumber % (sfTexture_getSize(vertex->tiles_text).x / 16);
            int tv = tileNumber / (sfTexture_getSize(vertex->tiles_text).x / 16);
            if (tileNumber == -1)
                continue;
